...
// Create page object
$students_add = new cstudents_add();
$Page =& $students_add;
...
// Create page object
var students_add = new ew_Page("students_add");

// page properties
students_add.PageID = "add"; // page ID
students_add.FormID = "fstudentsadd"; // form ID
var EW_PAGE_ID = students_add.PageID; // for backward compatibility

// extend page with ValidateForm function
students_add.ValidateForm = function(fobj) {
...
students_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
students_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
students_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
students_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $students->TableCaption() ?><br><br>
...
?>
<form name="fstudentsadd" id="fstudentsadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return students_add.ValidateForm(this);">
<p>
...
class cstudents_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $students->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$students->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $students->CurrentAction = "C"; // Copy record
		  } else {
		    $students->CurrentAction = "I"; // Display blank record
		...
		switch ($students->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$students->SendEmail = TRUE; // Send email on add success
		...
		$students->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $students;
		$students->studentImage->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($students->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		if ($students->portalUsername->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(portalUsername = '" . ew_AdjustSql($students->portalUsername->CurrentValue) . "')";
			$rsChk = $students->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		$bInsertRow = $students->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($students->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$students->studentID->setDbValue($conn->Insert_ID());
			$rsnew['studentID'] = $students->studentID->DbValue;

			// Call Row Inserted event
			$students->Row_Inserted($rsnew);
		}
