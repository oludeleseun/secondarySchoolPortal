...
// Create page object
$session_terms_add = new csession_terms_add();
$Page =& $session_terms_add;

// Page init
$session_terms_add->Page_Init();

// Page main
$session_terms_add->Page_Main();
?>
...
// Create page object
var session_terms_add = new ew_Page("session_terms_add");

// page properties
session_terms_add.PageID = "add"; // page ID
session_terms_add.FormID = "fsession_termsadd"; // form ID
var EW_PAGE_ID = session_terms_add.PageID; // for backward compatibility

// extend page with ValidateForm function
session_terms_add.ValidateForm = function(fobj) {
...
session_terms_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
session_terms_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
session_terms_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
session_terms_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $session_terms->TableCaption() ?><br><br>
<a href="<?php echo $session_terms->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$session_terms_add->ShowMessage();
?>
<form name="fsession_termsadd" id="fsession_termsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return session_terms_add.ValidateForm(this);">
<p>
...
<?php
$session_terms_add->Page_Terminate();
?>
<?php
...
class csession_terms_add {
	...
	// Page object name
	var $PageObjName = 'session_terms_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $session_terms->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$session_terms->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $session_terms->CurrentAction = "C"; // Copy record
		  } else {
		    $session_terms->CurrentAction = "I"; // Display blank record
		...
		switch ($session_terms->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$session_terms->SendEmail = TRUE; // Send email on add success
		...
		$session_terms->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($session_terms->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $session_terms->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($session_terms->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$session_terms->sessionTermID->setDbValue($conn->Insert_ID());
			$rsnew['sessionTermID'] = $session_terms->sessionTermID->DbValue;

			// Call Row Inserted event
			$session_terms->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
