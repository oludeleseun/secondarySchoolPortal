...
<input type="password" name="x_adminPassword" id="x_adminPassword" title="<?php echo $admin->adminPassword->FldTitle() ?>" size="20" maxlength="20"<?php echo $admin->adminPassword->EditAttributes() ?>>
...
class cadmin_add {
	...
	function AddRow() {
		...
		if ($admin->adminUsername->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(adminUsername = '" . ew_AdjustSql($admin->adminUsername->CurrentValue) . "')";
			$rsChk = $admin->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		if ($AddRow) {
			$admin->adminID->setDbValue($conn->Insert_ID());
			$rsnew['adminID'] = $admin->adminID->DbValue;
