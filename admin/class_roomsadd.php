...
// Create page object
$class_rooms_add = new cclass_rooms_add();
$Page =& $class_rooms_add;

// Page init
$class_rooms_add->Page_Init();

// Page main
$class_rooms_add->Page_Main();
?>
...
// Create page object
var class_rooms_add = new ew_Page("class_rooms_add");

// page properties
class_rooms_add.PageID = "add"; // page ID
class_rooms_add.FormID = "fclass_roomsadd"; // form ID
var EW_PAGE_ID = class_rooms_add.PageID; // for backward compatibility

// extend page with ValidateForm function
class_rooms_add.ValidateForm = function(fobj) {
...
class_rooms_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
class_rooms_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
class_rooms_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
class_rooms_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $class_rooms->TableCaption() ?><br><br>
<a href="<?php echo $class_rooms->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$class_rooms_add->ShowMessage();
?>
<form name="fclass_roomsadd" id="fclass_roomsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return class_rooms_add.ValidateForm(this);">
<p>
...
<?php
$class_rooms_add->Page_Terminate();
?>
<?php
...
class cclass_rooms_add {
	...
	// Page object name
	var $PageObjName = 'class_rooms_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $class_rooms->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$class_rooms->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $class_rooms->CurrentAction = "C"; // Copy record
		  } else {
		    $class_rooms->CurrentAction = "I"; // Display blank record
		...
		switch ($class_rooms->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$class_rooms->SendEmail = TRUE; // Send email on add success
		...
		$class_rooms->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($class_rooms->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $class_rooms->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($class_rooms->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$class_rooms->classID->setDbValue($conn->Insert_ID());
			$rsnew['classID'] = $class_rooms->classID->DbValue;

			// Call Row Inserted event
			$class_rooms->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
