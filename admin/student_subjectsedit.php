...
// Create page object
$student_subjects_edit = new cstudent_subjects_edit();
$Page =& $student_subjects_edit;

// Page init
$student_subjects_edit->Page_Init();

// Page main
$student_subjects_edit->Page_Main();
?>
...
// Create page object
var student_subjects_edit = new ew_Page("student_subjects_edit");

// page properties
student_subjects_edit.PageID = "edit"; // page ID
student_subjects_edit.FormID = "fstudent_subjectsedit"; // form ID
var EW_PAGE_ID = student_subjects_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
student_subjects_edit.ValidateForm = function(fobj) {
...
student_subjects_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
student_subjects_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
student_subjects_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
student_subjects_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $student_subjects->TableCaption() ?><br><br>
<a href="<?php echo $student_subjects->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$student_subjects_edit->ShowMessage();
?>
<form name="fstudent_subjectsedit" id="fstudent_subjectsedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return student_subjects_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="student_subjects">
...
<?php if ($student_subjects->studentSubjectID->Visible) { // studentSubjectID ?>
	<tr<?php echo $student_subjects->studentSubjectID->RowAttributes ?>>
		...
		<td<?php echo $student_subjects->studentSubjectID->CellAttributes() ?>><span id="el_studentSubjectID">
<div<?php echo $student_subjects->studentSubjectID->ViewAttributes() ?>><?php echo $student_subjects->studentSubjectID->EditValue ?></div><input type="hidden" name="x_studentSubjectID" id="x_studentSubjectID" value="<?php echo ew_HtmlEncode($student_subjects->studentSubjectID->CurrentValue) ?>">
</span><?php echo $student_subjects->studentSubjectID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$student_subjects_edit->Page_Terminate();
?>
<?php
...
class cstudent_subjects_edit {
	...
	// Page object name
	var $PageObjName = 'student_subjects_edit';
	...
	//
	function cstudent_subjects_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$student_subjects->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$student_subjects->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$student_subjects->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$student_subjects->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($student_subjects->studentSubjectID->CurrentValue == "")
			$this->Page_Terminate("student_subjectslist.php"); // Invalid key, return to list
		switch ($student_subjects->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$student_subjects->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$student_subjects->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($student_subjects->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// studentSubjectID
			$student_subjects->studentSubjectID->EditCustomAttributes = "";
			$student_subjects->studentSubjectID->EditValue = $student_subjects->studentSubjectID->CurrentValue;
			$student_subjects->studentSubjectID->CssStyle = "";
			$student_subjects->studentSubjectID->CssClass = "";
			$student_subjects->studentSubjectID->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $student_subjects->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($student_subjects->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$student_subjects->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
