...
// Create page object
$parents_add = new cparents_add();
$Page =& $parents_add;

// Page init
$parents_add->Page_Init();

// Page main
$parents_add->Page_Main();
?>
...
// Create page object
var parents_add = new ew_Page("parents_add");

// page properties
parents_add.PageID = "add"; // page ID
parents_add.FormID = "fparentsadd"; // form ID
var EW_PAGE_ID = parents_add.PageID; // for backward compatibility

// extend page with ValidateForm function
parents_add.ValidateForm = function(fobj) {
...
parents_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
parents_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
parents_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
parents_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $parents->TableCaption() ?><br><br>
<a href="<?php echo $parents->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$parents_add->ShowMessage();
?>
<form name="fparentsadd" id="fparentsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return parents_add.ValidateForm(this);">
<p>
...
<input type="password" name="x_portalPassword" id="x_portalPassword" title="<?php echo $parents->portalPassword->FldTitle() ?>" size="20" maxlength="20"<?php echo $parents->portalPassword->EditAttributes() ?>>
...
class cparents_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $parents->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$parents->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $parents->CurrentAction = "C"; // Copy record
		  } else {
		    $parents->CurrentAction = "I"; // Display blank record
		...
		switch ($parents->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$parents->SendEmail = TRUE; // Send email on add success
		...
		$parents->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($parents->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		if ($parents->portalUsername->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(portalUsername = '" . ew_AdjustSql($parents->portalUsername->CurrentValue) . "')";
			$rsChk = $parents->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		$bInsertRow = $parents->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($parents->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$parents->parentID->setDbValue($conn->Insert_ID());
			$rsnew['parentID'] = $parents->parentID->DbValue;

			// Call Row Inserted event
			$parents->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
