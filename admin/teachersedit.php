...
// Create page object
$teachers_edit = new cteachers_edit();
$Page =& $teachers_edit;
...
// Create page object
var teachers_edit = new ew_Page("teachers_edit");

// page properties
teachers_edit.PageID = "edit"; // page ID
teachers_edit.FormID = "fteachersedit"; // form ID
var EW_PAGE_ID = teachers_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
teachers_edit.ValidateForm = function(fobj) {
...
teachers_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
teachers_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
teachers_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
teachers_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $teachers->TableCaption() ?><br><br>
...
?>
<form name="fteachersedit" id="fteachersedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return teachers_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="teachers">
...
<?php if ($teachers->teacherID->Visible) { // teacherID ?>
	<tr<?php echo $teachers->teacherID->RowAttributes ?>>
		...
		<td<?php echo $teachers->teacherID->CellAttributes() ?>><span id="el_teacherID">
<div<?php echo $teachers->teacherID->ViewAttributes() ?>><?php echo $teachers->teacherID->EditValue ?></div><input type="hidden" name="x_teacherID" id="x_teacherID" value="<?php echo ew_HtmlEncode($teachers->teacherID->CurrentValue) ?>">
</span><?php echo $teachers->teacherID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<input type="password" name="x_portalPassword" id="x_portalPassword" title="<?php echo $teachers->portalPassword->FldTitle() ?>" value="<?php echo $teachers->portalPassword->EditValue ?>" size="20" maxlength="20"<?php echo $teachers->portalPassword->EditAttributes() ?>>
...
<div id="new_x_teacherImage">
<?php if (!empty($teachers->teacherImage->Upload->DbValue)) { ?>
<label><input type="radio" name="a_teacherImage" id="a_teacherImage" value="1" checked="checked"><?php echo $Language->Phrase("Keep") ?></label>&nbsp;
<label><input type="radio" name="a_teacherImage" id="a_teacherImage" value="2"><?php echo $Language->Phrase("Remove") ?></label>&nbsp;
<label><input type="radio" name="a_teacherImage" id="a_teacherImage" value="3"><?php echo $Language->Phrase("Replace") ?><br></label>
<?php $teachers->teacherImage->EditAttrs["onchange"] = "this.form.a_teacherImage[2].checked=true;" . @$teachers->teacherImage->EditAttrs["onchange"]; ?>
<?php } else { ?>
<input type="hidden" name="a_teacherImage" id="a_teacherImage" value="3">
<?php } ?>
...
class cteachers_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$teachers->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$teachers->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$teachers->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$teachers->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($teachers->teacherID->CurrentValue == "")
		...
		switch ($teachers->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$teachers->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$teachers->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($teachers->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// teacherID
			$teachers->teacherID->EditCustomAttributes = "";
			$teachers->teacherID->EditValue = $teachers->teacherID->CurrentValue;
			$teachers->teacherID->CssStyle = "";
			$teachers->teacherID->CssClass = "";
			$teachers->teacherID->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		$sFilter = $teachers->KeyFilter();
			...
			$sFilterChk = "(`username` = '" . ew_AdjustSql($teachers->username->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$teachers->CurrentFilter = $sFilterChk;
			$sSqlChk = $teachers->SQL();
		...
		if ($rs->EOF) {
			...
			$teachers->teacherImage->Upload->SaveToSession(); // Save file value to Session
						if ($teachers->teacherImage->Upload->Action == "2" || $teachers->teacherImage->Upload->Action == "3") { // Update/Remove
			$teachers->teacherImage->Upload->DbValue = $rs->fields('teacherImage'); // Get original value
			...
			$bUpdateRow = $teachers->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			if (!ew_Empty($teachers->teacherImage->Upload->Value)) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($teachers->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$teachers->Row_Updated($rsold, $rsnew);
		$rs->Close();
