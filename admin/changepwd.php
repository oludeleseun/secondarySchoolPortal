...
class cchangepwd {
	...
	//
	function Page_Main() {
		global $conn, $Language, $Security, $gsFormError, $admin;
		if (!ew_IsHttpPost())
			return;
		$bPwdUpdated = FALSE;
		...
		if ($this->ValidateForm($sOPwd, $sNPwd, $sCPwd)) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					if ((EW_MD5_PASSWORD && md5($sOPwd) == $rs->fields('adminPassword')) ||
						(!EW_MD5_PASSWORD && $sOPwd == $rs->fields('adminPassword'))) {
						$rsnew = array('adminPassword' => $sNPwd); // Change Password
						$rs->Close();
